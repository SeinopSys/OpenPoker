...
export const StatusIndicator: FC<LoadingIndicatorProps> = ({
  size = 18,
  ...
  const style = useMemo(
    () =>
      ({
        '--size': `${size / 16}rem`,
        '--animation-name': disableAnimation ? 'none' : undefined,
        '--fill-color': inheritColor ? 'currentColor' : undefined,
      }) as CSSProperties,
    [disableAnimation, inheritColor, size],
  );
  return (
    <svg
