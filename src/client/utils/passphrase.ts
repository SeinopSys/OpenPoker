 ...
 *
 * @see https://github.com/therootcompany/passphrase.js/blob/main/LICENSE
 */

export type TargetEntropy = 128 | 160 | 192 | 224 | 256;
type SeparatorCharacter = ' ' | '\t' | '\n' | ':' | ',' | '-';

interface PassphraseApi {
  generate(bitLen?: TargetEntropy, sep?: SeparatorCharacter): Promise<string>;

  encode(bytes: Uint8Array, sep?: SeparatorCharacter): Promise<string>;

  checksum(passphrase: string): Promise<boolean>;

  decode(passphrase: string): Promise<Uint8Array>;

  readonly base2048: string[];
}

const crypto = typeof window !== 'undefined' ? window.crypto : undefined;
const normalize = (str: string) => str.normalize('NFKD').trim().toLowerCase();
const sep = /[\s,:-]+/;

export const Passphrase: PassphraseApi = {
  /**
  ...
  /**
   * @param bitLen - The target entropy - must be 128, 160, 192, 224,
   *                 or 256 bits.
   * @param sep - Separator string between words
   * @returns The passphrase will be a space-delimited list of 12,
   *          15, 18, 21, or 24 words from the "base2048" word list
   *          dictionary.
   */
  async generate(
    bitLen: TargetEntropy = 128,
    sep?: SeparatorCharacter,
  ): Promise<string> {
    const byteLen = bitLen / 8;
    // ent
    const bytes = crypto.getRandomValues(new Uint8Array(byteLen));
    return await Passphrase.encode(bytes, sep);
  },

  /**
   * @param bytes - The bytes to encode as a word list
   * @param sep - Separator string between words
   * @returns The passphrase will be a space-delimited list of 12,
   *          15, 18, 21, or 24 words from the "base2048" word list
   *          dictionary.
  ...
  async encode(
    ...
    for (let bit = 0; bit < bitLen + sumBitLen; bit += 11) {
      ...
      const i = parseInt(digits.slice(bit, bit + 11).padStart(8, '0'), 2);
      seed.push(i);
    }

    const words = seed.map(function (i) {
  ...
  /**
   * @param passphrase - Same as from Passphrase.generate(...).
   * @returns True if the leftover checksum bits (4, 5, 6, 7, or 8)
   *          match the expected values.
   */
  async checksum(passphrase: string): Promise<boolean> {
    await Passphrase.decode(passphrase);
    return true;
  },

  /**
   * @param passphrase - The bytes to encode as a word list
   * @returns - The byte representation of the passphrase.
   */
  async decode(passphrase: string): Promise<Uint8Array> {
    passphrase = normalize(passphrase);

    // there must be 12, 15, 18, 21, or 24 words
    const ints = passphrase.split(sep).reduce(function (arr: number[], word) {
      // See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/normalize
      // 0-2047 (11-bit ints)
      const index = Passphrase.base2048.indexOf(word);
      if (index < 0) {
